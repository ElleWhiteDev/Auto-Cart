...
def migrate_multi_household():
    ...
    with app.app_context():
        ...
        try:
            recipes_without_household = Recipe.query.filter(Recipe.household_id.is_(None)).count()
            if recipes_without_household > 0:
                print(f"   ⚠ Found {recipes_without_household} recipes without household_id")
        ...
        try:
            lists_without_household = GroceryList.query.filter(GroceryList.household_id.is_(None)).count()
            if lists_without_household > 0:
                print(f"   ⚠ Found {lists_without_household} grocery lists without household_id")
        ...
        try:
            ...
            users_with_multiple = db.session.execute(text("""
            ...
            owners_with_multiple = db.session.execute(text("""
                SELECT COUNT(DISTINCT user_id) as count
                FROM household_members
                WHERE role = 'owner'
                GROUP BY user_id
            ...
            print(f"   Average households per user: {total_memberships / total_users if total_users > 0 else 0:.2f}")
        except Exception as e:
