...
class KrogerAPIService:
    ...
    def _safe_get_json_value(
        self, response_json: Dict[str, Any], key: str, default: Any = None
    ) -> Any:
    ...
    def refresh_access_token(
        ...
        if response and response.status_code == 200:
            ...
            new_refresh_token = self._safe_get_json_value(
    ...
    def get_profile_id(self, token: str) -> Optional[str]:
        ...
        logger.warning(
            f"Failed to get profile ID: {response.content if response else 'No response'}"
        )
        return None

    def get_stores(
        self, zipcode: str, token: str, limit: int = 5
    ...
    def search_products(
        self, ingredient: str, location_id: str, token: str, limit: int = 10
    ) -> Optional[Dict]:
    ...
    def add_items_to_cart(
        self, items: List[Dict], token: str, location_id: str = None
    ) -> bool:
        ...
        for item in items:
            ...
            # Add locationId if provided - this is CRITICAL for availability
            if location_id:
                prepared_item["locationId"] = location_id

            prepared_items.append(prepared_item)

        logger.debug(
            f"Adding {len(prepared_items)} items to cart for location {location_id}"
        )
        ...
        logger.error(
            f"Failed to add items to cart (status code: {response.status_code if response else 'No response'})"
        )
        return False
...
class KrogerSessionManager:
    ...
    def initialize_kroger_session():
        ...
        kroger_defaults = {"products_for_cart": [], "items_to_choose_from": []}
    ...
    @staticmethod
    def store_product_choices(
        products: List[Dict], current_ingredient_detail: Dict = None
    ):
    ...
    def add_product_to_cart(product_id: str, quantity: int = 1) -> bool:
        ...
        logger.debug(
            f"Current items_to_choose_from: {session.get('items_to_choose_from', [])}"
        )
        logger.debug(
            f"Current products_for_cart: {session.get('products_for_cart', [])}"
        )
        ...
        for item in session.get("items_to_choose_from", []):
            ...
            if item["id"] == product_id:
                ...
                session["items_to_choose_from"] = []
                logger.info(
                    f"Added {product_id} (qty: {quantity}) to cart. New cart: {session['products_for_cart']}"
                )
                return True
    ...
    def add_multiple_products_to_cart(
        product_ids: List[str], quantities: List[int]
    ) -> bool:
        ...
        logger.debug(
            f"Adding multiple products: {product_ids} with quantities: {quantities}"
        )

        items_dict = {
            item["id"]: item for item in session.get("items_to_choose_from", [])
        }
...
class KrogerWorkflow:
    ...
    def handle_authentication(
        ...
        if user.oauth_token and user.refresh_token:
            ...
            new_access_token, new_refresh_token = (
                self.kroger_service.refresh_access_token(user.refresh_token)
            )
        ...
        scope = "cart.basic:write product.compact profile.compact"
        auth_url = self.kroger_service.build_oauth_url(
            oauth_base_url, redirect_url, scope
        )
    ...
    def handle_callback(self, authorization_code: str, user, redirect_url: str) -> bool:
        ...
        try:
            access_token, refresh_token = self.kroger_service.get_access_token(
    ...
    def handle_product_search(self, token: str) -> str:
        ...
        if next_ingredient_detail:
            response = self.kroger_service.search_products(
                next_ingredient_detail["name"], session.get("location_id"), token
            )
            if response:
                products = parse_kroger_products(response)
                self.session_manager.store_product_choices(
    ...
    def handle_send_to_cart(self, token: str) -> str:
        ...
        logger.info(
            f"Sending {len(cart_products)} items to cart for location {location_id}"
        )
        ...
        for product in cart_products:
            ...
            if isinstance(product, dict):
                ...
                items.append({"quantity": 1, "upc": product, "modality": "PICKUP"})
    ...
    def ensure_valid_token(self, user) -> Optional[str]:
        ...
        new_access_token, new_refresh_token = self.kroger_service.refresh_access_token(
            user.refresh_token
        )
...
def parse_kroger_products(json_response: Dict) -> List[Dict[str, str]]:
    ...
    for product_data in products_data:
        ...
        if images:
            ...
            front_image = next(
                (img for img in images if img.get("perspective") == "front"), images[0]
            )
            ...
            if sizes:
                ...
                for preferred_size in size_preference:
                    size_obj = next(
                        (s for s in sizes if s.get("size") == preferred_size), None
                    )
                    if size_obj:
